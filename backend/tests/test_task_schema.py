...
def test_task_tables_are_registered_and_tenant_owned() -> None:
    ...
    weeks = Base.metadata.tables["project_weeks"]
    assert weeks.c.organization_id.nullable is False
    assert weeks.c.project_id.nullable is False
...
def test_tasks_use_tenant_qualified_project_assignee_and_actor_references() -> None:
    ...
    assert tasks.c.assignee_membership_id.nullable is True
    assert tasks.c.project_week_id.nullable is True
    assert tasks.c.required_skill_labels.nullable is False
...
def test_task_indexes_start_with_tenant_and_transition_is_append_only_shaped() -> None:
    ...
    assert ("organization_id", "project_week_id", "id") in indexes
